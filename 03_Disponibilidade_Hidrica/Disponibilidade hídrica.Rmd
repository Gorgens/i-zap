...
outorgasBacia$id = getOttoId$id
VazaoOtto = outorgasBacia@data %>%
        group_by(id) %>%
        summarise(VazaoAcum = sum(Vazao.Captada))

ottoInteresse = merge(ottoInteresse, VazaoOtto, by='id')

tm_shape(ottoInteresse) + tm_polygons('VazaoAcum') +
  tm_shape(outorgasBacia) + tm_dots("blue") +
        tm_grid(labels.inside.frame = FALSE, 
                projection = "+proj=longlat", col = 'gray')
...
redeHidrica = shapefile('../01_Delimitacao_e_Hidrografia/03_RedeHidrica_Bacia.shp')
proj4string(redeHidrica) = CRS("+init=epsg:4326")

getVazaoAcum = over(redeHidrica, ottoInteresse)
redeHidrica$VazaoAcum = getVazaoAcum$VazaoAcum


tm_shape(ottoMerged) + tm_polygons() +
  tm_shape(redeHidrica) + tm_lines('VazaoAcum') +
        tm_grid(labels.inside.frame = FALSE, 
                projection = "+proj=longlat", col = 'gray')
